...
fun PhoneScreen(
    ...
    LaunchedEffect(otpVerificationState) {
        when (otpVerificationState) {
            is Response.Success -> {
                isDialogVisible = false // Hide the dialog once OTP is successfully sent
                navController.navigate(Destinations.OtpScreen.route + "/$phoneNumber")
            }
            is Response.Error -> {
                isDialogVisible = false // Hide the dialog if there's an error
                Toast.makeText(context, otpVerificationState.message, Toast.LENGTH_SHORT).show()
            }
            else -> {
    ...
    Column(
        ...
        Button(
            ...
            onClick = {
                ...
                viewModel.createUserWithPhone(fullNumber, context as Activity)
            },
